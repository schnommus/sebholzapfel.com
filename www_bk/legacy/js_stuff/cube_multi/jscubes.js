...
// Currently showing cubes, but could potentially show anything
...
function Plot(plot, shape, offset) {
	...
	for( var i = 0; i != shape.length; ++i ) {
		...
		// Using a simple division for the projection transform - don't really need a matrix
		// for something this simple
		
		var px = Math.floor((shape[i].x + offset.x) / (shape[i].z+Z_OFF+offset.z)*V_DIST) + SIZE_Y/2;
		var py = Math.floor((shape[i].y + offset.y) / (shape[i].z+Z_OFF+offset.z)*V_DIST) + SIZE_X/2;
		
			
		if(i > 0) {
		  var x0 = px, y0 = py, x1 = opx, y1 = opy;
		  
		  // JS implementation of Bresenham's line algorithm ( curtosy of stackoverflow) begins:
...
// Perform an angular euler to matrix transform (aint that fun to say :D ), angles in radians.
...
// Was originally mouse-controlled - but then wasn't IE compatible
...
function loop() {
	...
	tCube = AngularTransform( CreateShape(18), cube, delta, delta, 0 );
	...
	for( var i = 0; i != cubes.length; ++i ) {
		...
		if(cubes[i].z < -10) { 
			cubes[i].z = 500;
			cubes[i].y = (Math.random()*SPAWN_Y)-SPAWN_Y/2;
			cubes[i].x = (Math.random()*SPAWN_X)-SPAWN_X/2;
		}
		Plot(plot, tCube, cubes[i]);
	}
...
function run() {
	...
	cubes = CreateShape(100);
	for( var i = 0; i != cubes.length; ++i ) {
		cubes[i].y = (Math.random()*SPAWN_Y)-SPAWN_Y/2;
		cubes[i].x = (Math.random()*SPAWN_X)-SPAWN_X/2;
		cubes[i].z = Math.random()*500+5;
	}
	loop();
}